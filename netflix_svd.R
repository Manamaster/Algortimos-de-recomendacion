...
head(dat.1)
nrow(dat.1)
#names(dat.1) <- c('peli_id','usuario_id_orig','calif','fecha')
#head(dat.1)

#usuarios <- unique(dat.1$usuario_id_orig)
## construir un consecutivo para usuarios (usamos después para matrices):
#usuarios.df <- data.frame(usuario_id_orig = usuarios, usuario_id = 1:length(usuarios) )
#dat.1 <- left_join(dat.1, usuarios.df)
#dat.1$usuarios_id_orig <- NULL
